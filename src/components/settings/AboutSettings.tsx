...
interface SocialLink {
  ...
  url: string;
  label: string;
  /** Couleur de marque du réseau — la seule couleur non fonctionnelle tolérée ici. */
  tint: string;
...
function SocialRow({ link }: { link: SocialLink }) {
  ...
  return (
    ...
    >
      ...
      <span className="ml-auto truncate text-xs text-muted-foreground">{t(`about.social.${link.id}`)}</span>
