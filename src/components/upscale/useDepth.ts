...
export function useDepth() {
  ...
  // Test sur une frame : orig = frame source, out = carte de profondeur.
  const testFrame = useCallback(async (time: number) => {
    if (!active || testing) return;
    setTesting(true); setTestErr(null);
    try {
      const r = await nr.processTestFrame({ input: active.path, time, mode: "depth", model: settings.model });
    ...
    try {
      for (let i = 0; i < sources.length; i++) {
        ...
        if (r.outputs) {
          ...
          reviews.push(...makeRenderReviews(src, r.outputs, "depth", segments));
        }
        imported += r.imported || 0;
        failed += r.failed || 0;
        if (!r.ok) lastErr = r.error || i18n.t("upscale:errors.depthFailed");
      }
