...
- Served to the grids through the Tauri **asset protocol** (`nr.assetUrl` → `convertFileSrc`), never `/media`. See below.

**Grid previews never go through the core's HTTP server.** `/media` shares its origin with `/rpc` and the SSE `/events` stream, and a Chromium webview opens at most **6 HTTP/1.1 connections per origin**: `/events` holds one for the whole session, in-flight RPCs take more, and the few that remain are shared by every `<video>` and `<img>` of the grid. A grid whose proxies are *all* already encoded still played only a handful of cards at a time — the files existed, they were queued behind connection slots. `nr.assetUrl()` returns a `convertFileSrc()` URL instead: the request is intercepted in-process by the Rust shell (`assetProtocol` in `tauri.conf.json`, scope `**` because the cache folder is relocatable at runtime), with no socket and no per-origin pool. Outside Tauri (Adobe CEP panel, plain browser) it falls back to `/media`, which is why the fallback must stay. Thumbnails follow the same route — hundreds of `<img>` competed for those same 6 slots.

**A grid card never asks the core for a proxy it could already know.** `ffmpeg:proxy` is a request to *encode*: it queues behind `proxyGate`, rewrites the sidecar, and is cancelled when the card leaves the band — so a scroll fired one round trip per card, plus one cancellation each, for files that were all already on disk, and the grid stayed on its thumbnails. `nr.proxyResolve()` resolves a whole shot list in **one** call from a single `readdir` of the proxy folder (the name is an md5 of source+range+encoder+tier, and a file only bears that name once its atomic rename is done), so every card finds its URL in the renderer cache before it reaches the screen. It is the proxy counterpart of `thumbsResolve`, and **every** preview grid calls it — cutting, collections, live timeline and search — whenever its shot list or its density changes. Search keeps its own card hook (`useResultPreview`), so the two must be kept in step: shared `PLAY_LEAD_PX`, preload band derived from it, synchronous URL read, no slot churn on hover. `test/rush-grid-resize-perf.test.cjs` fails when one drifts from the other. When the wanted tier is missing but another one exists, the other one is returned: a slightly softer preview beats a frozen thumbnail while ffmpeg runs. **Resolving counts as using**: automatic cleanup is an LRU over `used` (`cachePolicy`), refreshed only by `cacheIndex().touch` — since grids no longer go through `proxySegment` for a cache hit, the resolve is the sole remaining proof that a file is still in service, and without that touch the proxies of a rush you watch daily would be purged as abandoned.
...
Preview audio (`PreviewVideo`, prop `audible`): muted by default, but the **hovered** card plays sound at the adjustable `hoverVolume` (default 20%). Autoplaying previews stay muted. Two budgets share Chromium's ~75 media players per frame and must be read together: `MAX_PLAYING_HARD` (52, `cutStudioShared`) and `MAX_PAUSED` (18, `previewVideoPool`). Past that limit the browser silently stops creating players, so the bottom of a dense grid freezes on its thumbnail with no error to show for it — locked by `test/rush-grid-resize-perf.test.cjs`.

**A card mounts its `<video>` before it is allowed to play.** The element is created as soon as the card enters the *preload* band (`PLAY_LEAD_PX + 300`), paused and hidden behind the thumbnail, so the file is read and the decoder initialised while the card is still approaching; the play slot then only lifts the pause and the first frame is immediate. Mounting on the grant instead cost a load plus a decoder init at the exact moment the card reached the screen — the "arrives frozen, then starts" effect. The lead distance itself is **one constant** (`PLAY_LEAD_PX`): the observer band and `autoplayCeiling`'s row count both derive from it, and when they drifted apart, rows claimed a slot the ceiling had never budgeted for. Slots are granted on `requestAnimationFrame`, in small bursts — a timer does its work *between* frames, competing with the scroll it is meant to serve. `muted` is driven imperatively (the JSX attribute stays constant so autoplay is always allowed), with a muted fallback if Chromium blocks audible autoplay. Because player volume 0 acts as a global mute, Settings ▸ Media ▸ Previews exposes **both** sliders and shows "muted by player volume" — the page used to show only the hover slider, greyed out, with no way to fix it.
...
## Durable mirror of the interface settings

`core/uistate.js` + `src/lib/uiState.ts`, tested by `test/ui-state.test.cjs`. Everything the user tunes in the interface (theme, wallpaper, volumes, columns, cut shortcuts, module defaults…) lives in `localStorage`, which belongs to the app's **WebView2 profile**: a recreated profile, a storage cleanup or a different origin brought the whole app back to its defaults and every setting had to be entered again.

The core therefore keeps a string key→value copy on disk (`NR_HOME/ui-state.json`, atomic write, SSE `uistate:changed` broadcasting the **patch**), and the renderer mirrors every `nr.`/`nr-` key into it — `nr.auth.*` excluded, since an authentication grace tied to one profile must not outlive it. A value over 512 KB is refused: that is a dataset, not a setting.

Two rules make it safe:

- **Hydration runs before the first render and before `App` is imported** (`main.tsx`): the store reads `localStorage` while its module is evaluated, so loading it earlier would start the app on the defaults of an emptied storage. `BootFrame` is what stands in until then, and imports neither the store nor any module that reads `localStorage` on evaluation. Core unreachable (4 s timeout) → the app starts on local storage, exactly as before the mirror.
- **Hydration never deletes.** Disk wins for the keys it knows; a local key it does not know is *seeded* into it, because it comes from a version older than the mirror. Deletions only travel live, through the broadcast patch.

`core/prefs.js` (above) stays separate: it carries the typed **work** settings and replays them hot into the other windows' store. The mirror ignores the shape of the values and only touches `localStorage` — an incoming patch is applied there for the next mount, without re-rendering.

## File drops come in as HTML5 DnD, and must never be left to the WebView
