...
`SetupGate` wraps the shell (not the detached window). At boot the renderer calls `setup:status`; if the venv or ffmpeg is missing it shows the install screen, and `setup:run` launches `scripts/setup.ps1`, which provisions the CUDA torch venv, the requirements, a GPL ffmpeg build with `hevc_nvenc`, and the default weights (~2.5 GB, best effort). The ffmpeg archive is a **prerelease asset of this repository**, pinned by version and SHA-256 and served by GitHub's CDN; keeping this auxiliary release marked as a prerelease prevents it from replacing the NetsuRush updater's `/releases/latest/` target. It is produced from an upstream build by [`scripts/ffmpeg-mirror.ps1`](../scripts/ffmpeg-mirror.ps1) (shared build, `ffmpeg` + `ffprobe` + their DLLs, zip). The fallback is a BtbN zip, also on GitHub — no source outside a CDN, and no external extractor. Progress is streamed over SSE with `STAGE:`/`PROGRESS:`/`ERROR:` markers, and the config file is written at the end. **A restart is required afterwards**, since the config is read when the core starts. The browser mock always reports ready, so the gate never appears outside the app. `setup.ps1` is idempotent. With no system Python, a standalone build is downloaded.
...
- **Unattended install is opt-in and launch-only** (`nr.update.autoInstall`, off unless turned on in Settings ▸ Updates). It downloads and restarts with no click, but only on the version the *boot* probe found — a manual check keeps its two steps — only once `App.tsx` has armed it from the shell, so a relaunch can never cut the first-run provisioning or an in-flight sign-in, and only if the download lands within 120 s of that arming: past the launch window the bytes wait for the title-bar button rather than closing the window on a running export. Turning off auto-check turns it off too: there is no probe left to feed it.
...
- **Session is optional.** It travels as the `Better-Auth-Cookie` header of the crossDomain plugin, since the desktop webview holds no cookie on the Convex domain, and it only names the reporter in the embed — a signed-out tester can still send, because the broken sign-in is itself a bug worth reporting. Quota: 10 reports per hour (`convex/bugs.ts`) per account, or per salted IP fingerprint (`BUG_QUOTA_SALT`) when anonymous — the raw address is never stored. Recorded **after** Discord accepts, so failed sends do not consume it.
