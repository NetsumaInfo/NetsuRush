...
- **Cutting and extraction are frame-exact**: stream copy (`-c copy`) only when `core/export/frameCut.js` proves the cut lands clean on the source packets, precise re-encode otherwise. Encoders are **never hard-coded per vendor**: the proxy and export paths resolve a probed hardware encoder (NVENC, AMF or Quick Sync) and fall back to hardware H.264, then to `libx264`. Never trigger `libx265` automatically. Details in `docs/invariants.md`.
